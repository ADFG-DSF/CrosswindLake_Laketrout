...
```{r}
plot_fig4 <- function(tab, ...) {
  par(family="serif")
  p_winter <- seq(from=0, to=1, length.out=nrow(tab))
  plot(p_winter, tab$YPn, type="l", ylim=range(0, tab$upr95_YPn),
...
$$\hat{V}_{naive}(MSY_n) = \frac{MSY^2}{\bar{W}^4}\hat{V}(\bar{W})$$  **We did not do this.**

where

$\hat{W}$ = estimated mean weight of lake trout subject to harvest from Crosswind Lake.

**I used the Bayesian model to estimate $W_{\infty}$, using all our catch sampling plus info from other studies, including the length ~ age relationship observed for Crosswind and hierarchical info from 40 other lakes.  This gave a SE around $W_{\infty}$ rather than treating it as known.  Also - I dug into Lester and found some additional sources of variance that were approximated using Monte Carlo simulation, but I still need to write this up.**
...
fig5_ci <- data.frame(ci_lo=rep(NA, length(p_winter)), ci_hi=NA)
for(i in seq_along(p_winter)) {
...
```{r}
plot_fig5 <- function(est, ci, ...) {
  par(family="serif")
  p_winter <- seq(from=0, to=1, length.out=nrow(est))
  plot(p_winter, est$MSY50, type="l", ylim=range(0,ci),
...
tbl8 <- data.frame(perc_MSY=100*p_msy, MSY_n=NA, SE_MSY_n=NA, Upr_95_CI=NA, Lwr_95_CI=NA)
